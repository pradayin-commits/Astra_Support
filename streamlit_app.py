...
# ==========================================
# 3. ENHANCED DIALOGS (BOSCH STANDARDS)
# ==========================================
    ...
    with st.form("create_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Submit to Astra", use_container_width=True):
            if not title.strip() or not rep.strip():
                st.error("Summary and Reported By are required fields.")
            else:
                ...
                st.cache_data.clear()
                st.success("✅ Defect registered successfully.")
                st.rerun()

@st.dialog("✏️ Modify Defect Record")
def edit_defect_dialog(record):
    with st.form("edit_form"):
        st.markdown(f"### 📑 Editing Record: {record['id']}")
        st.caption("Modifying this record will update the global registry for all users.")
        ...
        if col_s.form_submit_button("💾 Commit Changes", use_container_width=True):
            with get_engine().begin() as conn:
                conn.execute(text("""
        ...
        if col_c.form_submit_button("✖️ Discard Changes", use_container_width=True):
            st.session_state.editing_id = None
            st.rerun()
...
with tab_insights:
    if not df.empty:
        # (Standard Analytics Code as before)
        st.subheader("Analytical Overview")
        d1, d2, d3 = st.columns(3)
        cat_1 = d1.selectbox("Filter Category", ["module", "priority", "status"])
        val_1 = d2.selectbox(f"Select {cat_1.title()}", ["All Data"] + sorted(df[cat_1].unique().tolist()))
        ...
        g2.plotly_chart(px.bar(c_df.groupby(cat_2).size().reset_index(name='Cnt'), x=cat_2, y='Cnt', color=cat_2, title="Count"), use_container_width=True)

with tab_explorer:
    st.subheader("Defect Table") # Requested Heading
    
    # BOSCH STANDARD: Explicit Interaction Help
    st.info("""
        **User Instructions for Data Modification:**
        1. Locate the record in the table below.
        2. Click the **Selection Circle** on the left side of the row.
        3. An automated **Modification Dialog** will open to process your changes.
    """)

    search = st.text_input("🔍 Quick Search Registry", placeholder="Search by Summary, ID, or Module...")
    ...
    event = st.dataframe(
        ...
        column_config={
            "id": st.column_config.TextColumn("ID", help="Unique system identifier"),
            "defect_title": st.column_config.TextColumn("Summary", width="large"),
            "created_at": st.column_config.DatetimeColumn("Reported Date", format="DD.MM.YYYY HH:mm")
        }
    )
    ...
    if event and event.selection.rows:
        ...
        st.session_state.editing_id = disp_df.at[sel_idx, 'id']
        st.toast(f"Loading Record ID: {st.session_state.editing_id}", icon="📑")
